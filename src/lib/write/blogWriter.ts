...
- 본문 안에 소제목을 넣을 위치마다 그 줄 맨 앞에 "## "를 붙이세요 (예: "## 첫인상은 이랬어요").
  소제목 개수·주기는 아래 유형별 규칙(있다면)을 따르세요.
- 본문 중 강조하고 싶은 짧은 문구(핵심 정보·숫자·결론 등)는 "**문구**"처럼 별표 두 개로
  감싸세요. 문단마다 많아야 1~2곳 — 남발하면 오히려 안 읽힙니다.
- 본문 중 사용자가 올린 사진을 넣으면 좋을 위치마다 정확히 "[사진N]" 형태로 표시하세요
  (N은 1부터 시작하는 업로드 순서). **사용자가 사진을 한 장도 올리지 않았다면 이 표시를
  절대 쓰지 마세요.**
- 사용자가 올린 사진 외에 보완할 무료 스톡 사진이 있으면 좋을 위치마다 "[스톡이미지]"
  표시를 넣으세요(번호 없이, 필요한 만큼 여러 번 사용 가능). 이 검색어는
  "stockImageQueries" 배열에 3~4개, 반드시 영어 키워드 구문으로 담으세요(스톡 이미지
  검색 서비스가 영어 태그 위주라서).
- 실제 사진이나 스톡 사진으로 대체하기 어려운 경우(제품 비교표, 데이터 시각화, 개념
  설명용 그래픽 등)에만 AI로 새로 생성한 이미지를 요청하세요 — 비용이 드는 기능이니
  꼭 필요할 때만 0~2개 이내로 사용하세요. 본문에는 "[AI이미지N]"(N은 1부터) 표시를
  넣고, 그 이미지를 그릴 상세한 영어 프롬프트를 "aiImagePrompts" 배열에 순서대로
  담으세요(이미지 생성 모델은 영어 프롬프트에서 품질이 가장 좋습니다).
...

{
  ...
  "body": "본문 전체 (문단 구분은 줄바꿈 두 번, ## 소제목, **강조**, [사진N]/[스톡이미지]/[AI이미지N] 표시 포함)",
  ...
  "aiImagePrompts": ["detailed english prompt for a generated image, e.g. a product comparison chart"]
}`;
