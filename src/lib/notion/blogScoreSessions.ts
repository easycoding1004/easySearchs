...
function parseSession(page: PageObjectResponse): BlogScoreSession {
  ...
  const businessName = richText(props[BLOG_SCORE_SESSION_PROPS.businessName]);
...
export async function createBlogScoreSession(input: {
  ...
  const page = await notion.pages.create({
    ...
    properties: {
      ...
      ...(input.insightReport
        ? {
            [BLOG_SCORE_SESSION_PROPS.insightReport]: {
              type: "rich_text" as const,
              rich_text: [{ type: "text" as const, text: { content: input.insightReport.slice(0, 1900) } }],
            },
          }
        : {}),
      ...(input.authorId
