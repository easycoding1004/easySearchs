...
- **서식 포함 복사 — 실측으로 이미지는 안 된다는 게 확인됨(2026-07)**: 표준 `navigator.clipboard.write([new ClipboardItem({...})])`로 `text/html`+`text/plain`을 함께 씀 — 서버 자동화가 아니라 브라우저 표준 Clipboard API. 굵게/소제목/목록 서식은 네이버 에디터 붙여넣기에서 유지되지만, `<img>`는 style을 어떻게 넣어도 통째로 사라짐(사용자가 실제 붙여넣기로 확인) — 그래서 `renderBodyToHtml()`은 이제 이미지를 아예 embed하지 않고 `<mark>` 안내 문구만 남기고, `BlogWriterForm.tsx`의 화면 미리보기(붙여넣기와 무관, 우리 페이지 안)에 이미지마다 "이 사진 다운로드" 링크를 달아서 사용자가 직접 받아 끼워 넣게 함. **이 결론은 실측으로 확정된 것이니 다시 `<img>`를 clipboard HTML에 넣어보는 시도를 반복하지 말 것.**
- **태그 — 일괄 붙여넣기가 아니라 한 개씩 클릭 복사로 바뀜(2026-07, 실측 확인)**: 네이버 태그 입력창은 쉼표/Enter를 "키 입력 이벤트"로만 태그 구분자로 인식하고, 붙여넣은 텍스트에 있는 쉼표는 그냥 글자로 취급해서 `tag1,tag2`가 태그 하나로 들어감 — 어떤 구분자(쉼표/줄바꿈/공백)로 포맷을 바꿔도 안 풀리는, 대상 필드 자체의 한계. `BlogWriterForm.tsx`가 태그 배지 각각을 버튼으로 만들어 클릭 시 그 태그 하나만(쉼표·`#` 없이) 복사하도록 바꿈(`handleCopySingleTag`) — 사용자가 하나씩 클릭→붙여넣기→Enter를 반복.
- **AI 이미지 생성**: `src/lib/write/generateAiImages.ts`가 OpenAI Images API(`POST https://api.openai.com/v1/images/generations`, `gpt-image-1`, 공식 문서로 스펙 확인 후 구현)를 호출, `data[].b64_json`을 `data:image/png;base64,...`로 바로 씀. Pixabay와 같은 계약(`OPENAI_API_KEY` 없거나 실패해도 throw 안 하고 `null`) + **입력 prompts와 항상 1:1 길이 유지**(실패분을 배열에서 걸러내면 `[AI이미지N]` 번호가 밀려서 엉뚱한 이미지가 매칭됨). 이미지 1장당 실제 과금됨 — Claude/Pixabay보다 비쌈. 개수 상한은 2026-07에 2→5로 상향(제품 여러 개 비교처럼 맥락상 여러 장이 실제 필요한 경우를 막지 말아달라는 요청) — `parseResult()`의 `.slice(0, 5)`가 최종 하드 캡. 프롬프트는 구도/스타일은 영어, 이미지 안에 실제로 렌더링될 글자는 반드시 한국어로 인용 + "번역하지 말라" 지시 — 처음엔 전체를 영어로만 쓰게 했다가 이미지 속 글자까지 영어로 나오는 문제가 있어서 고침(`gpt-image-1` 실제 호출로 재검증 완료, 한글 정상 렌더링 확인).
- **사진 선택사항화 + 개수/용량 상한**: `/api/write`의 "사진 1장 이상 필수" 검증 제거. 사진 0장이면 `recommendedThumbnail`은 0(센티널, "없음")으로 강제(`parseResult`), UI는 `recommendedThumbnail > 0`일 때만 추천 썸네일을 보여줌. 장수는 2026-07에 5→10으로 상향했지만 **장당 용량 캡과 별도로 "전체 합계" 캡(18MB)이 반드시 같이 있어야 함** — Claude API 요청 전체 크기 한도가 32MB(base64 포함, base64는 원본의 약 4/3배)라서, 장당 캡만 있으면 10장을 각각 최대치로 채웠을 때 그 한도를 넘어 요청 자체가 거절됨. 장수를 더 올리려면 이 총합 캡과 장당 캡을 같이 재계산할 것.
- **생성된 글 수정 요청**: `POST /api/write/revise`(하루 1회 제한과 무관, 대신 글 하나당 `MAX_REVISIONS`(5)회로 별도 캡, 서버가 클라이언트가 보낸 `revisionCount` 재검증)가 `blogWriter.ts`의 `reviseBlogPost()`를 호출 — `generateBlogPost`와 실제 Claude 호출부(`callClaude`)를 공유하고 user 턴 텍스트만 다름("기존 제목+본문+태그를 통째로 다시 보내고 수정 요청만 반영, 나머지는 유지하라"). API가 대화 상태를 유지하지 않는 단발 요청이라 매번 전체 맥락을 다시 보내야 함. 유형(category)은 재분류 안 하고 최초 생성 값을 그대로 유지.
