...
- **웹크롤링 자동 수집 병행**(사용자 요청, 2026-08) — **실측 테스트 결과 4개 주요 플랫폼 모두 직접 스크래핑이 불가능함을 확인, 구현하지 않기로 결론**. 실제 요청으로 확인한 결과(§10.4 "추측 금지, 실측 확인 후 진행" 원칙 그대로):
  ...
  - **11번가**: HTTP 200으로 응답은 오지만(위 두 곳과 달리 즉시 차단은 아님), 실제 페이지가 `<div id="App"></div>` 하나뿐인 클라이언트 렌더링 SPA라 상품·가격 데이터가 전부 JS 실행 이후에만 채워짐 — `fetch`/`curl` 같은 단순 HTTP 요청으로는 애초에 데이터 자체가 없음. 실제로 가져오려면 헤드리스 브라우저(Puppeteer/Playwright) 렌더링이 필요한데, 이는 이 프로젝트가 지금까지 한 어떤 스크래핑(§10.4의 블로그·카페 프로필 페이지, 전부 서버사이드 렌더링된 정적 HTML)보다 훨씬 무거운 인프라이고, 주요 이커머스 플랫폼은 이런 자동화 접근 자체를 약관으로 금지하는 경우가 많아 §10.4가 지금까지 승인해온 것보다 리스크 등급이 다름.
  - **결론**: 4곳 다 직접 스크래핑으로는 안 되는 것으로 실측 확인됨 — 추가로 시도하지 말 것. 회원 등록 방식(21.1~21.3)이 유일하게 검증된 경로.
