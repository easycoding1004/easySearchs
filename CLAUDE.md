...
- **Claude 호출**: `src/lib/write/blogWriter.ts` — `@anthropic-ai/sdk`로 이미지 base64 + 프롬프트를 한 메시지에 담아 전송, 응답은 JSON(title/body/recommendedThumbnail/thumbnailReason)만 오도록 시스템 프롬프트로 강제하고 방어적으로 파싱(앞뒤에 다른 텍스트가 붙어도 첫 `{`~마지막 `}`만 추출). `thumbnailReason`은 Claude가 가끔 빈 문자열로 줄 때가 있어서(실측 확인) UI에서 비어있으면 "— 이유" 부분 자체를 안 보여주게 처리.
- 필요 환경변수: `ANTHROPIC_API_KEY`(console.anthropic.com에서 별도 발급 — Claude Code/Claude.ai 구독과 무관, **실제 키 발급받아 설정 완료**), `NOTION_USERS_DB_ID`(`scripts/setup-notion-users.ts`로 생성 완료, 소셜 로그인용 속성은 `scripts/add-user-social-login-props.ts`로 추가 완료), `AUTH_EMAIL_FROM`(인증 메일 발신 표시명), `NAVER_LOGIN_CLIENT_ID`/`NAVER_LOGIN_CLIENT_SECRET`(developers.naver.com 앱 등록, 검색광고/오픈API 키와 별개 앱 — **아직 미발급**), `KAKAO_CLIENT_ID`/`KAKAO_CLIENT_SECRET`(developers.kakao.com 앱 등록, Client Secret은 카카오 앱에서 켰을 때만 필요 — **아직 미발급**). 네이버/카카오 둘 다 Redirect URI를 `https://ezzsearch.com/api/auth/{provider}/callback`로 정확히 등록해야 함.
- **검증 완료 (실제 프로덕션 standalone 서버 + 실제 Notion/Resend/Claude API로)**: 회원가입→이메일 발송→인증→로그인→세션 쿠키→**실제 사진을 업로드해 Claude가 정말 이미지를 보고 이해한 자연스러운 글을 생성**(로고 이미지로 테스트, `[사진1]` 위치 표시 포함)→일일 사용 플래그 세팅→재요청 시 429 차단까지 전부 실측 확인. 네이버/카카오 로그인은 Client ID/Secret 발급 전이라 설정 안 됐을 때 503으로 정상적으로 막히는 것까지만 확인 — 실제 OAuth 동의 화면을 거치는 전체 흐름은 사용자가 각 개발자 센터에서 앱을 등록한 뒤 브라우저로 직접 확인 필요(코드로 재현 불가능한 부분).
