...
**범위 결정 (사용자와 논의)**: 원래 구상은 세 갈래(① 실시간 오버레이 — 네이버 검색결과·블로그 에디터·스마트스토어·플레이스 페이지에 content script로 데이터 얹기, ② 원클릭 접근 — 우클릭 조회/팝업/주소창 검색, ③ 자동 모니터링 — 관심 키워드 등록 시 급변 알림 + Notion 자동 적재)였으나, **이번엔 ②만 구현**하기로 확정함. 이유:
- ①은 네이버 페이지의 실제 DOM 구조에 의존하는 content script라, 네이버가 마크업만 바꿔도 계속 깨지는 유지보수 부담이 큼.
